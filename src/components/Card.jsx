...
export default function Card(props) {
    ...
    return (
        <div className={`${props.width} shrink-0 mb-3`}>
            ...
            <div className="image-overlay absolute w-full h-full top-0 flex items-end p-2 text-[16px] md:text-[23px] font-bold text-white trancing-tighter">{props.offer}</div>
            </div>
            <div className="mt-3 text-md md:text-xl font-bold">{props.title}</div>
            <div><Star className="inline"/> {props.rating}
            <span className="ml-2">{props.maxTime}-{props.minTime} mins</span>
